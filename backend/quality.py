...
def compute_rule_wqi(values: dict):
    ...
    wqi = (temp_score*WEIGHTS['Temperature'] +
           ph_score*WEIGHTS['pH'] +
...
def update_models():
    ...
    essential_cols = ["Temperature", "pH", "BOD", "Faecal_Coliform", "Total_Coliform",
                      "Nitrate", "Conductivity", "WQI", "Anomaly_Status"]
    df = df.dropna(subset=essential_cols)
    if df.empty:
        return

    X = df[["Temperature", "pH", "BOD", "Faecal_Coliform", "Total_Coliform", "Nitrate", "Conductivity"]]
...
# ==============================
# 🔹 GET: All Quality Records for an MC
# ==============================
@router.get("/mc/{mc_code}/quality-records")
def get_quality_records(mc_code: str, hub_id: str = None):
    query = """
...
# ==============================
# 🔹 GET: Trend Summary per MC / Hub (safe + future-proof)
# ==============================
@router.get("/mc/{MC_Code}/trend")
def get_quality_trend(MC_Code: str, Hub_ID: str = None):
    query = """
    ...
    # Drop rows without Created_At or WQI
    ...
    # Build trend summary safely (avoid warning)
    trend_summary = (
        df.groupby("Hub_ID", group_keys=False)  # ✅ silences FutureWarning
        .apply(lambda x: {
...
# ==============================
# 🔹 GET: Anomaly Summary per MC / Hub
# ==============================
@router.get("/mc/{MC_Code}/anomalies")
def get_anomaly_summary(MC_Code: str, Hub_ID: str = None):
    query = """
        SELECT Hub_ID, WQI, Anomaly_Status, Created_At
        FROM water_quality_records
        WHERE MC_Code = :MC_Code
    ...
    return {
        ...
        "Records": records,
        "Message": "✅ No anomalies detected" if len(records) == 0 else f"⚠️ {len(records)} anomalies detected"
    }

# ==============================
# 🔹 GET: Yearly / Hub Trend Summary with Trend Direction + Yearly Delta
# ==============================
@router.get("/mc/{MC_Code}/yearly-trend")
def get_yearly_trend(MC_Code: str, Hub_ID: str = None):
    query = """
        SELECT Hub_ID, WQI, Anomaly_Status, Created_At
        FROM water_quality_records
        WHERE MC_Code = :MC_Code
